...
import { useLifeDeskSnapshot, useLifeDeskStore, getTasksForDate, getPersonById } from "@/store/useLifeDeskStore";
            ...
            tasks.map((task) => {
              ...
              return (
                <div key={task.id} className="rounded-[24px] border border-[color:var(--border-soft)] bg-[color:var(--panel-muted)] p-4">
                  ...
                  <p className="mt-2 text-sm text-[color:var(--text-soft)]">{task.moduleType === "social" && person ? `${language === "zh-CN" ? "人物" : "Person"}：${person.name}` : task.moduleType === "event" ? (language === "zh-CN" ? "事件模块" : "Event module") : ""}</p>
